...
class point:
    ...
    def avoid_collision(self,min_list):
		...
		yellow_cone=[]
		blue_cone=[]
		if min_list[0]].flag==0:
			yellow_cone.append(sorted_colorcone[i])
		else:
			blue_cone.append(sorted_colorcone[i])
		if(len(yellow_cone)==1):   #파랑이면 좌조향
	...
	def circle(self):
		...
		if len(self.obData.circles) == 0:
			...
			if(len(circles)==2):
				...
				if (len(yellow_cone)==1 or len(yellow_cone)==2) and len(blue_cone)==0 : #(self.two_circle_distance>3.0 and left_circle.center.x + 0.4 < right_circle.center.x and self.two_between > 90):
				...
				if(min_list[0]<1.3) : #if(min_list[0]<1.3 and abs(min_list[1].y)<1.1):
